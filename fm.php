    ...
    foreach ($items as $item) {
        ...
        if (is_dir($source)) {
            ...
            foreach ($files as $file) {
                $file = realpath($file);
                $relativePath = basename($source) . '/' . str_replace($source . '/', '', $file);
                if (is_dir($file)) {
...
// Global scope helper for recursive PHP deletion
...
if (isset($_POST['bulk_action']) && !empty($_POST['selected'])) {
    ...
    if ($action === 'delete') {
        ...
        foreach ($selectedItems as $item) {
            $targetPath = $dir . DIRECTORY_SEPARATOR . $item;
            if (purgePath($targetPath)) $deletedCount++;
        }
        $msg = "Successfully purged $deletedCount selected item(s).";
    } else if ($action === 'download') {
        $zipName = 'batch_download_' . time() . '.zip';
        $tmpZip = sys_get_temp_dir() . DIRECTORY_SEPARATOR . $zipName;

        if (zipItems($selectedItems, $dir, $tmpZip)) {
            ...
            $err_msg = "Failed to bundle selected items into a ZIP package.";
        }
    }
}
...
if (isset($_GET['download'])) {
    ...
    if (file_exists($targetPath)) {
        if (is_dir($targetPath)) {
            ...
            $tmpZip = sys_get_temp_dir() . DIRECTORY_SEPARATOR . $zipName;
            if (zipItems([$targetName], $dir, $tmpZip)) {
                header('Content-Type: application/zip');
                header('Content-Disposition: attachment; filename="' . $zipName . '"');
                header('Content-Length: ' . filesize($tmpZip));
                readfile($tmpZip);
...
if (isset($_GET['delete'])) {
    ...
    if (file_exists($targetPath) && $targetName !== '.' && $targetName !== '..') {
        if (purgePath($targetPath)) {
            $msg = "Successfully purged " . htmlspecialchars($targetName);
        } else {
            $err_msg = "Deletion blocked by server permissions.";
        }
    }
}
...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <!-- Main Form Wrapper for Batch Actions -->
        <form method="post" id="fileForm" class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div class="p-4 border-b border-slate-800 bg-slate-900/50 flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    <button type="submit" name="bulk_action" value="download" id="btnBulkDownload" disabled class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-sky-500/20 bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 transition text-xs font-semibold disabled:opacity-40 disabled:cursor-not-allowed">
                        <i data-lucide="archive-restore" class="w-3.5 h-3.5"></i> Zip Selected
                    </button>
                    <button type="submit" name="bulk_action" value="delete" id="btnBulkDelete" disabled onclick="return confirm('Purge selected items immediately?')" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-rose-500/20 bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 transition text-xs font-semibold disabled:opacity-40 disabled:cursor-not-allowed">
                        <i data-lucide="trash" class="w-3.5 h-3.5"></i> Delete Selected
                    </button>
                </div>
            </div>
                    ...
                    <thead>
                        <tr class="border-b border-slate-800 bg-slate-950/60 text-slate-400 uppercase font-semibold">
                            <th class="py-3.5 px-4 w-10 text-center">
                                <input type="checkbox" id="selectAll" class="rounded border-slate-800 bg-slate-950 text-indigo-600 focus:ring-0 w-4 h-4 cursor-pointer">
                            </th>
                        ...
                        ?>
                            <tr class="hover:bg-slate-800/40 transition group">
                                <td class="py-3 px-4 text-center">
                                    <input type="checkbox" name="selected[]" value="<?= htmlspecialchars($f) ?>" class="item-checkbox rounded border-slate-800 bg-slate-950 text-indigo-600 focus:ring-0 w-4 h-4 cursor-pointer">
                                </td>
